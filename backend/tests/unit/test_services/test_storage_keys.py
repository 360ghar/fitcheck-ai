...
class TestMintKey:
    def test_canonical_shape(self):
        ...
        assert re.fullmatch(rf"{USER}/items/[0-9a-f]{{32}}\.png", key)
        assert key.startswith(f"{USER}/items/")
...
class TestMintExportKey:
    def test_shape(self):
        assert mint_export_key(USER) == f"{USER}/export/data.json"
...
# --------------------------------------------------------------------------- #
# thumb_key_for — derived thumbnail siblings
# --------------------------------------------------------------------------- #
class TestThumbKeyFor:
    def test_derives_thumb_sibling(self):
    ...
    def test_thumb_is_always_webp(self):
        for ext in ("jpg", "jpeg", "png", "avif"):
            assert thumb_key_for(f"{USER}/avatars/{HEX32}.{ext}").endswith("_thumb.webp")
    ...
    def test_returns_none_for_thumb_of_thumb(self):
        assert thumb_key_for(f"{USER}/items/{HEX32}_thumb.webp") is None

    def test_returns_none_for_extensionless_and_empty(self):
        assert thumb_key_for(f"{USER}/items/{HEX32}") is None
...
class TestParseKey:
    ...
    def test_thumb(self):
        ref = parse_key(f"{USER}/outfits/{HEX32}_thumb.webp")
    ...
    def test_preview(self):
        ref = parse_key(f"tmp/{USER}/social-import/{HEX32}.webp")
    ...
    def test_export(self):
        ref = parse_key(f"{USER}/export/data.json")
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "   ",
            None,
            f"{USER}/tmp/batch/not-hex.webp",
            f"{USER}/items/{HEX32}.txt",
            "items/abc.png",
            f"{USER}/export/data.json/extra",
