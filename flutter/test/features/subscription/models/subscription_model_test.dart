...
void main() {
  ...
  test(
    'refunded status parses without throwing (admin mark-refunded flow)',
    () {
  ...
  test('store products parse and resolve per-variant product IDs', () {
    final products = StoreProductsModel.fromJson({
      ...
      'google': {'plus_monthly': 'com.fitcheck.plus.monthly'},
    });

    expect(
      products.productIdFor('apple', 'plus_monthly'),
      'com.fitcheck.plus.monthly',
    );
    expect(products.productIdFor('apple', 'pro_monthly'), isNull);
    expect(
      products.productIdFor('google', 'plus_monthly'),
      'com.fitcheck.plus.monthly',
    );
  ...
  test(
    'store products never fall back when only some variants are configured',
    () {
      final products = StoreProductsModel.fromJson({
        'apple': {
          'plus_monthly': 'com.fitcheck.plus.monthly',
  ...
  test('plans response defaults billing_configured to false', () {
    final response = PlansResponse.fromJson({'plans': []});
  ...
  test('plans response bundles plans with store products', () {
    ...
    expect(
      response.storeProducts.productIdFor('apple', 'plus_monthly'),
      'com.fitcheck.plus.monthly',
    );
