...
- **A concurrency rejection is overload, not a provider failure.** Bodies naming the gateway's own cap are retryable: non-2xx rejections retry via the transient path, and 200 envelopes carrying the marker with no images retry inside the image POST wrapper; both back off ~2 s (a sub-second retry just re-enters the full queue), and are recorded via `record_result(..., overload=True)`: they neither advance nor reset the consecutive-failure streak, and install a short 10 s cooldown instead. Overload admission is evaluated before the 60 s health TTL; the jittered breaker cooldown is clamped to the 300 s cap.
