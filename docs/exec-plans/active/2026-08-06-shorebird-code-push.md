...
| 2026-08-08 | iOS dry-run (`build_ios_release.sh --dry-run`): archive built + signed OK (automatic signing, team HMWGCVU4SV), but the IPA export failed with `exportArchive No Accounts with App Store Connect Access` — Xcode on this Mac has no account with ASC access. Fix is a human step: sign in to Xcode (Settings → Accounts) with the team's Apple ID, or install the App Store distribution profile + use manual signing (as CI does), or use CI once the Apple secrets are added |
