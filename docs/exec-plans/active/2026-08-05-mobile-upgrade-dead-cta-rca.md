...
| 2026-08-08 | Third follow-up (message truthfulness + store-state UX). Verified from the plugin source (`in_app_purchase_storekit` 0.4.11, the StoreKit 2 path this app uses by default): `storekit_no_response` is NOT a network failure — the Dart wrapper throws it when the native side returns a *successful empty result* for a non-empty ID list (`sk2_product_wrapper.dart`); a thrown `Product.products(for:)` (genuine network/server failure) surfaces as `storekit2_products_error` instead. So "the store couldn't be reached" was factually wrong for the dominant case: the store WAS reached and resolved zero products (ASC setup incomplete). Fixes landed: (1) `fetchProducts` maps `storekit_no_response` to `kPlanNotAvailableInStoreMessage` ("This plan is not available in the store yet. Please check back soon.") and keeps "store couldn't be reached / try again in a moment" only for real store errors; (2) the controller now tracks `StoreStatus` (unknown/ready/notConfigured/unavailable) from `refreshStoreProducts` — only the definitive zero-products failure (`storekit_no_response` or a success with zero products) marks `unavailable`; transient store errors leave `unknown` so tap-time retries still apply; (3) `_startStorePurchase` fails fast with the accurate message when the page load already proved the store unavailable (no pointless re-query + retry delay at the tap); (4) the paywall renders a slim "Upgrades aren't available in the store yet" banner + Retry above the plan cards (mobile only) that self-heals once ASC starts serving the products — no app restart needed; (5) `retryStoreProducts` shows a "Store ready" confirmation when the rail comes up. Analyzer clean; 198 flutter tests green (new coverage: per-code message mapping, genuine-error retry message, status transitions incl. transient-unknown and unavailable-preservation, fail-fast checkout, banner render + Retry recovery). Real fix remains the App Store Connect checklist (ops step 5). |
