...
- **Wardrobe Page:** `frontend/src/pages/wardrobe/WardrobePage.tsx`
  ...
  - Upload + AI extraction via `frontend/src/components/wardrobe/ItemUpload.tsx` → `BatchExtractionFlow.tsx`
  - Job state: `frontend/src/hooks/useBatchExtraction.ts` + SSE; compress: `lib/image-compress.ts`
  - Background progress: `jobUiStore` + AppLayout `JobPill`
...
- `backend/app/api/v1/items.py`
  - `POST /api/v1/items/upload` (store images after review)
  - CRUD: `GET/POST/PUT/DELETE /api/v1/items`
  - Actions: favorite, wear, categorize, image management
- `backend/app/api/v1/batch_processing.py`
  - `POST /api/v1/ai/batch-extract-multipart` (web preferred)
  - `POST /api/v1/ai/batch-extract` (JSON base64; Flutter)
  - SSE: `GET /api/v1/ai/batch-extract/{job_id}/events`
  - Status / cancel endpoints for reconnect and soft-close

### AI (Server-Side via Backend API)

Batch wardrobe upload pipeline:
- Orchestration: `backend/app/services/batch_extraction_service.py` (extract parallel; **generation overlaps** extract)
- Job store: `backend/app/services/batch_job_service.py`
- Agents: `item_extraction_agent.py`, `image_generation_agent.py`
- One-shot helpers: `POST /api/v1/ai/extract-items`, `POST /api/v1/ai/generate-product-image`
- Providers: Gemini, OpenAI, or custom proxy (per-user configurable)
