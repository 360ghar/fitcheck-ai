...
## Batch AI Extraction (async jobs + SSE)

Primary wardrobe upload path. Clients create a job, then subscribe to SSE for progress.

**Pipeline behavior:**
1. Client may compress images (longest edge ≤ ~1568px, JPEG quality ~0.85) before upload.
2. Job runs in the background after `202 Accepted`.
3. **Extraction:** all images processed in parallel (server concurrency limit).
4. **Generation (if `auto_generate`):** product images start as soon as items from an image are detected — **overlapped** with remaining extractions (not a strict “all extract, then all generate” barrier).
5. `total_items` on generation events can **grow** as more images finish extraction.
6. Terminal events: `all_generations_complete` (when auto-generate is on and gen ran) then `job_complete`.

### POST /ai/batch-extract

Start a batch job with **JSON base64 images** (Flutter and legacy clients).
...
```json
{
  "images": [
    {
      "image_id": "client-unique-id",
      "image_base64": "<base64 without data-url prefix>",
      "filename": "photo.jpg"
    }
  ],
  "auto_generate": true,
...
| Field | Notes |
|-------|--------|
| `images` | 1–50 items; each `image_base64` max ~10MB encoded |
| `auto_generate` | Default `true` — generate studio product images |
| `generation_batch_size` | 1–5 concurrent product generations |
...
```json
{
  ...
  "status": "pending",
  "total_images": 2,
  "sse_url": "/api/v1/ai/batch-extract/{job_id}/events",
  "message": "Batch extraction started for 2 images"
}
```

### POST /ai/batch-extract-multipart

Start a batch job with **multipart file upload** (preferred for web).

**Request (multipart/form-data):**
```
files: <image>[, <image>...]     # 1–50 files, field name "files"
image_ids: '["id1","id2"]'       # JSON array string, parallel to files (optional)
auto_generate: true|false
generation_batch_size: 1-5
```

Same **202** response shape as JSON start. Binary upload avoids base64 wire bloat; server still prepares base64 for the vision pipeline.

### GET /ai/batch-extract/{job_id}/events

SSE stream (auth via header or query token where EventSource cannot set headers).

**Event types:**
| Event | Meaning |
|-------|---------|
| `connected` | Subscriber attached |
| `heartbeat` | Keep-alive |
| `extraction_started` | Extract phase began |
| `image_extraction_complete` | One image finished extract (`items`, counts) |
| `image_extraction_failed` | One image failed extract |
| `all_extractions_complete` | All images finished extract |
| `generation_started` | Product gen began (**may fire before** `all_extractions_complete`) |
| `item_generation_complete` | One studio image ready (`generated_image_base64`, counts) |
| `item_generation_failed` | One studio image failed |
| `all_generations_complete` | All queued generations finished |
| `job_complete` | Job finished successfully (includes items snapshot) |
| `job_failed` / `job_cancelled` | Terminal failure / cancel |

### GET /ai/batch-extract/{job_id}/status

Snapshot for reconnect / poll fallback (`items`, extraction and generation counts, error).

### POST /ai/batch-extract/{job_id}/cancel

Best-effort cancel of a running job.

### POST /ai/single-extract

Single-image async job (same job/SSE infrastructure). Optional 24h extraction cache unless `skip_cache`.
